...
1. workflow 更强调真实执行结构
   `prompts/workflow-generation.md` 现在会优先区分 `serial_procedure`、`iterative_loop`、`phase_serial`、`phase_with_internal_steps` 等模式，避免把串行 phase 错画成并列分支。

2. `full` 翻译更强调真实性
   最终合并阶段会拦截占位文本、英文整句回填、伪中文后缀和缺少中文主体的“假完整翻译”。

3. 摘要提取更稳
   `normalize-skill.mjs` 现在会正确解析 folded frontmatter，例如 `description: >`，避免 hero 摘要退化成 `>` 或默认占位语句。

4. 翻译阅读体验更好
   渲染层保留逐行校验真源，同时把连续 prose 行合并成段落、把列表项合并成列表，HTML 阅读体验更接近真实文档。
...
`prompts/workflow-generation.md` 还会提示模型先判断 workflow family，再选择更贴近 skill 原文的图法，尤其区分：
...
这让 workflow 既能保留 phase 对应关系，也更容易侧挂脚本、模板和 reference 依赖。
workflow 标签会尽量保留来源中的关键原语言术语，例如 `baseline`、`dry_run`、`git revert`、`Result Card`，避免为了中文化而损失语义精度。
`workflow.nodes[*].id` 只作为内部引用键使用，不应直接透传到 Mermaid。渲染时必须先映射成 Mermaid-safe id，避免 `style`、`class` 等保留字或语法敏感 token 导致整张图解析失败。
...
- `compact`
  适合常规快速检查，保留少量关键段落的双语内容。
- `full`
  适合需要逐段中英对照的英文或非中文来源。
- `summary`
  适合原文已经主要是中文的 skill；第二主区块显示“中文总结”，不再展示原文对照。
...
- `full` 模式下必须保持 section 和 row 与英文骨架一一对应
- 占位文本、英文整句复制、`（中文）` 之类伪翻译会在 finalize 阶段直接失败
- 命令、路径、URL、变量、frontmatter key 仍然允许保留英文，但要嵌入中文句法中
- 渲染时连续 prose 会合并成段落，列表项会保留为列表块

翻译层约束：

- `translation.sections[*]` 必须使用 `title_zh`、`title_en`、`rows[*].zh`、`rows[*].en`
- 需要中英对照时，每一行中文都要保留对应英文 `en`
- `summary` 模式下，`rows[*].zh` 承载中文总结，`rows[*].en` 可以为空
...
- 生成 `reportSeeds`
- 解析 folded frontmatter，例如 `description: >`
- 为摘要生成更稳定的 `summary.purpose`
...
- 校验最终 report shape
- 对 `full` 翻译执行真实性校验，拦截 placeholder、英文回填和伪中文后缀
- 拒绝使用默认兜底摘要作为最终 `summary.purpose`
...
- 把 report 中的各个区块填入模板
- 在 Mermaid 渲染前把 workflow id 映射成安全节点 id，避免保留字冲突
- 把翻译区的连续 prose 行聚合成段落，把列表行聚合成列表块
...
The latest iteration added four important upgrades:

1. More execution-faithful workflow graphs
   `prompts/workflow-generation.md` now steers models to distinguish `serial_procedure`, `iterative_loop`, `phase_serial`, and `phase_with_internal_steps` instead of collapsing everything into one generic graph style.

2. Stronger anti-fake translation checks for `full`
   Finalization now rejects placeholder rows, copied English prose, fake Chinese suffix markers, and rows that preserve tokens but fail to translate the natural-language content.

3. More reliable summary extraction
   `normalize-skill.mjs` now parses folded frontmatter such as `description: >` correctly, so the hero summary no longer degrades into `>` or a fallback placeholder.

4. Better translation reading experience
   The renderer keeps row-level validation as the source of truth but presents continuous prose as paragraphs and bullet-like rows as lists in HTML.
...
`prompts/workflow-generation.md` now also nudges models to pick a workflow mode before drawing the graph, especially for:

- plain serial action chains
- explicit phase backbones
- phase backbones with key internal steps
- reference-driven dependency layouts

This improves generalization by guiding graph selection rather than only tightening output constraints.
...
Current `full` translation guarantees:

- section and row alignment must match the English draft skeleton
- placeholder text, copied English prose, and fake suffixes such as `（中文）` are rejected during finalization
- commands, paths, URLs, variables, and frontmatter keys may remain in English, but the surrounding sentence must still be real Chinese
- rendered HTML groups continuous prose into paragraphs and preserves lists as list blocks
...
- parsing folded frontmatter descriptions safely
- generating a more stable `summary.purpose`
...
- rejecting fake-complete `full` translations
- rejecting fallback hero summaries as final output
...
- grouping translation prose into paragraph blocks and list blocks for reading
