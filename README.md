...
- `out/normalized-source.json`
- `out/report.draft.json`
- `out/report.overlay.template.json`
- `out/report.json`
- `out/report.html`
...
- `full_auto`
  直接复用 draft 的英文 section 骨架
- `full_human`
  overlay 只补中文 `zh`，英文 `en` 从 draft 继承
- `full_override`
  overlay 同时重写 `zh` 与 `en`
...
node scripts/render-report.mjs out/report.json out/report.html
```
...
Current overlay coverage modes:
