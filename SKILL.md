...
1. Resolve the input into raw skill text
2. Run `node scripts/normalize-skill.mjs <input>`
3. Run `node scripts/build-report-draft.mjs skill-inspector/<skill-name>/normalized-source.json`
4. Run `node scripts/build-report-overlay-template.mjs skill-inspector/<skill-name>/report.draft.json`
5. Fill a small `report.overlay.json` for judgment-heavy fields only
6. Run `node scripts/finalize-report.mjs skill-inspector/<skill-name>/report.draft.json skill-inspector/<skill-name>/report.overlay.json`
7. Validate the final `report.json` against `templates/report.schema.json`
8. Render `report.html` from that validated `report.json`
9. Re-open both outputs and verify they still match
...
- When the source is a remote link that still resolves to HTML, an empty body, or inaccessible content after fallback, stop and report the fetch failure instead of fabricating analysis fields
...
JSON correction rules:

- When helper scripts are available, prefer repairing `normalized-source.json`, `report.draft.json`, or `report.overlay.json` through the local script pipeline instead of freehand reshaping the final report
- When a helper script is unavailable or fails, continue manually with the same canonical `report.json` schema and record the relevant limitation in `safety` or `suggestions` when it affects confidence
- When required template or schema files are missing, stop and report the missing path rather than inventing replacement structures
...
- Do not guess missing source text, missing references, or missing translation rows in order to force a complete report
...
- Stop the run when `report.json` cannot be written or does not pass schema validation
...
- When one of the helper scripts is missing or fails, keep the same artifact order: `normalized-source.json` first when possible, then schema-shaped `report.json`, then `report.html`
- When you must continue manually, preserve the same field names, translation rules, workflow structure, and write order defined above

Post-write verification:

- Re-open the written `report.json` and verify it still matches `templates/report.schema.json`
- Re-open the written `report.html` and verify the major sections reflect the same workflow, translation, install, score, and suggestions content as `report.json`
- When HTML embeds JSON for client-side rendering, verify the embedded payload parses successfully
- When the written artifacts drift from each other, regenerate `report.html` from the same final `report.json` instead of editing the HTML separately

Completion checklist:

- Confirm `report.json` is the canonical final artifact
- Confirm `report.html` mirrors the same workflow, translation, install, score, and suggestions content
- Confirm every non-deterministic claim in `references`, `safety`, `score`, and `suggestions` is supported by source evidence
- Confirm translation mode requirements are satisfied without placeholders or fake bilingual rows
- Confirm the final response points the user to the generated artifact paths

Failure handling and stop conditions:

- If the input path does not exist, is unreadable, or resolves to non-Markdown content, stop and report that input problem before any report drafting
- If normalization succeeds but evidence is too thin to support a claimed reference, safety finding, or score explanation, remove the unsupported claim instead of padding it with generic prose
- If `workflow.nodes` or `workflow.edges` stay incomplete after normalization and overlay work, keep correcting the workflow data until it is structurally valid or stop with a clear limitation report
- If translation cannot satisfy the declared `translation.mode`, keep the English skeleton and report the translation gap clearly instead of mixing partial placeholder content into final output
